...
export class EmployersService {
  ...
  async getEmployers({
    ...
    return this.getEmployersItemsPayloadDto({
      limit: +limit,
      offset: +offset,
    });
  }
  ...
  async updateEmployerId({
    ...
    await this.findEmployerById(id);
  ...
  async findEmployerById(id: number): Promise<Employer> {
  ...
  async getEmployersItemsPayloadDto({
    limit = 0,
    offset = 100,
  }: {
    limit: number;
  ...
  async isExistEmployerData({
    ...
    const isExistTelegram: Employer | null =
      await this.prismaService.employer.findUnique({
        where: {
          telegram,
        },
